...
app.get(`/get/author/:author`, async (req, resp) => {
  let data = await BlogsModel.find({ author: req.params.author }).populate(
    "author"
  );
  resp.send(data);
});
...
app.get(`/get/genre/:genre`, async (req, resp) => {
  let data = await BlogsModel.find({ genre: req.params.genre }).populate(
    "author"
  );
  resp.send(data);
});
...
app.get(`/get/:title`, async (req, resp) => {
  let data = await BlogsModel.find({ title: req.params.title }).populate(
    "author"
  );
  resp.send(data);
});
